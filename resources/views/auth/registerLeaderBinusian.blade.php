<x-guest-layout>
    <form method="POST" action="{{ route('/binusian-register') }}" enctype="multipart/form-data">
        @csrf
        ...
        <!-- Flazz Card -->
        <div>
            <x-input-label for="flazzCard" :value="__('Flazz Card (Binusian)')" />
            <x-text-input id="flazzCard" class="block mt-1 w-full" type="file" name="flazzCard" :value="old('flazzCard')" required autofocus autocomplete="flazzCard" />
            @error('flazzCard') <span class="error">{{ $message }}</span> @enderror
        </div>
